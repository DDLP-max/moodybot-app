/**
 * Structure prompts for Dynamic Mode tone routing.
 * Ported from the Replit Telegram bot (structure_prompts.py).
 */
export const STRUCTURE_PROMPTS: Record<string, string> = {
  ...
  "/mirror": "Reflect the user's words back with sharper edges. Structure: Echo → Reframe → Soft rupture. Make them hear themselves clearly.",
  "/clinical": "Analytical and structured. Structure: Name the pattern → Map the mechanism → Offer a clean cut of insight. No fluff. No therapy-speak.",
  "/copywriter": "Apply Ogilvy clarity with MoodyBot bite. Structure: Sharp hook → Benefit-first body → CTA that punches. Stay vivid and zero-fluff.",
  "/dale-yolo": "Crash-mode truth. Structure: Name the spiral → Cut the performance → Leave a raw human landing. Chaotic energy, honest core.",
};

/** Resolve a mode/persona name to a STRUCTURE_PROMPTS entry. */
export function getStructurePrompt(mode: string): string | undefined {
  const key = mode.trim().toLowerCase().startsWith("/")
    ? mode.trim().toLowerCase()
    : `/${mode.trim().toLowerCase().replace(/\s+/g, "-").replace(/\//g, "-")}`;
  return STRUCTURE_PROMPTS[key];
}
