...
export function lensVoiceGuidance(lens: string): string {
  ...
  if (lens === "Emotional Intelligence") {
    return [
      ...
      "APPROACH DIVERSITY (craft, not routing): same mechanism, different doors. Do NOT always open with \"It isn't really about X…\".",
      "Openings: Observation / Contradiction / Image / Irony / Reversal / Relocation (sometimes). Endings: punch, drift, or stop — not always \"revealing the speaker.\"",
      'FAIL (formula): every EI reply = relocate → mechanism → reveal speaker.',
